(() => {
    ...
    function init() {
      els = {
        ...
        nextFridayBtn: document.getElementById("nextFridayBtn"),
    ...
    function bindGlobalEvents() {
      ...
      els.nextFridayBtn.addEventListener("click", () => {
        setPeriodToNextFriday();
      });
      
      els.reportPeriod.addEventListener("input", () => {
        collectSettings();
        updateDefaultEmailSubject();
      });      
    ...
    function updateDefaultEmailSubject() {
        const period = els.reportPeriod.value.trim();
      
        els.emailSubject.value = period
          ? `Relatório semanal - Journée - ${period}`
          : "Relatório semanal - Journée";
    }
    ...
    function setPeriodToNextFriday() {
        const today = new Date();
        
        // Meio-dia evita problemas raros de alteração de horário de verão.
        today.setHours(12, 0, 0, 0);
        
        // Domingo = 0, segunda = 1, ... sexta = 5, sábado = 6.
        const daysUntilFriday = (5 - today.getDay() + 7) % 7;
        
        today.setDate(today.getDate() + daysUntilFriday);
        
        els.reportPeriod.value = formatDateBR(today);
        
        collectSettings();
        updateDefaultEmailSubject();
        
        toast("Período ajustado para a próxima sexta-feira");
    }     
    ...
    function renderDay() {
      ...
      day.locations.forEach((location, index) => {
        ...
        const isCollapsed = collapsedLocationIds.has(location._id);

        card.className = isCollapsed
          ...
          <div class="location-head">
            <div class="location-title">
                ...
                <span class="collapsed-summary">
                    | Duração de ${escapeHtml(compactDuration(location.duration))}
                    ± ${Number(location.duration_variation || 0)}min
                    | Deslocamento de ${escapeHtml(compactDuration(location.travel))}
                    ± ${Number(location.travel_variation || 0)}min
                </span>
            </div>
            ...
            <div class="location-actions">
                <button
                    ...
                    title="${isCollapsed ? "Restaurar edição do local" : "Minimizar local"}"
                    aria-label="${isCollapsed ? "Restaurar edição do local" : "Minimizar local"}"
                >
                    ${isCollapsed ? "▸" : "▾"}
                </button>
    ...
    function bindLocationCardEvents(card, location, index, day) {
      ...
      collapseBtn.addEventListener("click", () => {
        ...
        if (collapsedLocationIds.has(location._id)) {
          collapsedLocationIds.delete(location._id);
        } else {
          collapsedLocationIds.add(location._id);
        }
